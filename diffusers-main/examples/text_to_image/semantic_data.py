...
def load_data(
    *,
    ...
    if dataset_mode == 'cityscapes':
        ...
        classes = [x for x in labels_file if x.endswith('_color.png')]
    ...
    def __getitem__(self, idx):
        ...
        if self.local_instances is not None:
            ...
            with bf.BlobFile(instance_path, "rb") as f:
                ...
                pil_instance.load()
            pil_instance = pil_instance.convert("RGB")
        else:
            pil_instance = None
        ...
        if arr_instance is not None:
            ...
            out_dict['instance'] = np.transpose(arr_instance, [2, 0, 1])
...
def resize_arr(pil_list, image_size, keep_aspect=True):
    ...
    pil_instance = pil_instance.resize((224, 224)) if pil_instance is not None else None
