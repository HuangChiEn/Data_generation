...
from taming.models.vae import VAESub
...
class VAEModel(pl.LightningModule):
    ...
    def forward(self, input, segmap=None):
        dec, posterior = self.vae(input, segmap)
    ...
    def training_step(self, batch, batch_idx):
        ...
        xrec, posterior = self(x, y)
        opt_ae, opt_disc = self.optimizers()

        # autoencode
        # train encoder+decoder+logvar
        aeloss, log_dict_ae = self.loss(x, xrec, posterior, 0, self.global_step,
        ...
        # discriminator
        discloss, log_dict_disc = self.loss(x, xrec, posterior, 1, self.global_step,
                                            last_layer=self.get_last_layer(), split="train")
