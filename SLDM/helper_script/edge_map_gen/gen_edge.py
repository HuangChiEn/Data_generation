...
def instances2dict_with_polygons(imageFileList, verbose=False):
    ...
    for imageFileName in imageFileList:
        ...
        # init zero map
        total_edge_map = np.zeros_like(imgNp)

        # Loop through all instance ids in instance image
        for instanceId in np.unique(imgNp):
            # empty mask
            edge_map = np.zeros_like(total_edge_map)

            instanceObj = Instance(imgNp, instanceId)
            instanceObj_dict = instanceObj.toDict()

            #print(id2label[instanceObj.labelID].name)

            # where's the source of id2label got from
            #if id2label[instanceObj.labelID].hasInstances:
            #    print('has inst')
            mask = (imgNp == instanceId).astype(np.uint8)
            edge_map = cv2.Canny(mask, 0, 1)

            total_edge_map += edge_map
            
        kernel = np.ones((3,3), np.uint8)
        # bolder the edge ~
        total_edge_map = cv2.dilate(
            total_edge_map.astype('uint8'), 
            kernel, 
            iterations=1
        ).astype('int32')  # turn back to original dtype
        
        sav_path = imageFileName.replace('_instanceIds', '_edgeMaps')  #.split('/')[-1]
        #sav_path = './tmp/' + sav_path
        
        #breakpoint()
        cv2.imwrite(sav_path, total_edge_map)
...
def convert_cityscapes_instance_only(data_dir, out_dir):
    ...
    print(f"total : {len(tst_lst)}")
    print(cnt)

def parse_args():
    parser = argparse.ArgumentParser(description='Convert dataset')
    parser.add_argument('--dataset', help="cityscapes", default='cityscapes', type=str)
    parser.add_argument('--outdir', help="output dir for json files", default='./tmp', type=str)
    parser.add_argument('--datadir', help="data dir for annotations to be converted", default="/data1/dataset/Cityscapes", type=str)
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
